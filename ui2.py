...
canvas0.pack()  # 放置canvas的位置

canvas1 = Canvas(window,bg="whitesmoke", width=1024, height=164)
frm_top = Frame(canvas1)  # 把frame放在canvas里
...
eva =''
# Entry(frm2, textvariable = modelPath, bg='whitesmoke',relief=RAISED).pack(side=TOP,anchor=W, fill=BOTH,pady=2,padx=2)
# Entry(frm2, textvariable = userPath, bg='whitesmoke',relief=RAISED).pack(side=TOP,anchor=W, fill= BOTH,pady=2,padx=2)
# Entry(frm2, textvariable = modelVideoPath, bg='whitesmoke',relief=RAISED).pack(side=TOP,anchor=W, fill=BOTH,pady=2,padx=2)
# Entry(frm2, textvariable = userVideoPath, bg='whitesmoke',relief=RAISED).pack(side=TOP,anchor=W, fill=BOTH,pady=2,padx=2)
...
# frm3=Frame(frm_top)
# Button(frm3, text = "choose file", command = selectModelPath, bg='whitesmoke',relief=RAISED).pack(side=TOP,anchor=W,pady=5,padx=2,fill=Y)
# Button(frm3, text = "choose file", command = selectUserPath, bg='whitesmoke').pack(side=TOP,anchor=W,pady=5,padx=2,fill=Y)
# Button(frm3, text = "choose file", command = selectModelVideoPath, bg='whitesmoke').pack(side=TOP,anchor=W,pady=5,padx=2,fill=Y)
# Button(frm3, text = "choose file", command = selectUserVideoPath, bg='whitesmoke').pack(side=TOP,anchor=W,pady=5,padx=2,fill=Y)
...
# #imgPath= '/Users/xitingliang/Desktop/test2.png'
# img_open1 = Image.open('D:\study\hand_gesture\-++n\openpose-1.4.0-win64-gpu-binaries\openpose-1.4.0-win64-gpu-binaries\compare\sample\\right/1.jpg').resize((100, 100))
# img_png1 = ImageTk.PhotoImage(img_open1)
# label_img1 = Label(window, image = img_png1)
# label_img1.pack(side=LEFT)
# #changePic(label_img1)
# img_open2 = Image.open('D:\study\hand_gesture\-++n\openpose-1.4.0-win64-gpu-binaries\openpose-1.4.0-win64-gpu-binaries\compare\sample\\wrong/2.jpg').resize((100, 100))
# img_png2 = ImageTk.PhotoImage(img_open2)
# label_img2 = Label(window, image = img_png2)
...
def print_selection():
    ...
    #     picIndexArry.append(temp)
    #     temp = temp+6
    # for i in picIndexArry:
    #     print("我做到了")
    #     currentModelImgAddr = modelPicDir + str(i) +'.jpg'
    #     currentUserImgAddr = userPicDir + str(i) + '.jpg'
